#!/usr/bin/env node
// Round 5B.3 HF3-HF1 IWER harness: Three.js controller-object association
// plus HF3 per-hand Trigger source fidelity. This is not Quest visual proof.
...
const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), 'halcyon-jp4a-r5b3-hf3-hf1-'));
const port = Number(process.env.HALCYON_JP4A_PORT || 17437);
...
async function waitForServer() {
  ...
  throw new Error('JP4A HF3-HF1 harness server timeout');
}
...
try {
  ...
  await page.screenshot({ path: path.join(outDir, 'jp4a-round5b3-hf3-hf1-console.png') });
  await page.evaluate(() => {
  ...
  const firstRun = await page.evaluate(async () => {
    ...
    const seam = 'jp4a_controller_connected_lifecycle_not_inputSources_index';
    ...
    return {
      entered, firstWorldRender: !!world, seam, association,
  ...
  const evidence = {
    ...
    scope: 'CONTROLLER_OBJECT_CONNECTED_LIFECYCLE_AND_REORDERED_INPUTSOURCES',
    ...
    iwerRayPickBoundary: 'Headless IWER cannot prove Quest physical controller pose or a native reconnect/reorder. This run injects Three.js connected/disconnected on actual controller objects 0/1, then simulates a reordered WebXR active list [LEFT, RIGHT] through the same logical updateControllers path. NOT_HARDWARE_VISUAL_PROOF.',
    before,
    firstRun: {
  ...
  fs.writeFileSync(path.join(outDir, 'jp4a-round5b3-hf3-hf1-iwer.json'), JSON.stringify(evidence, null, 2));
  console.log(JSON.stringify({
    pass,
    evidence: 'docs/review/jp4a/jp4a-round5b3-hf3-hf1-iwer.json',
    liveShelfInvariant,
    associationOk,
