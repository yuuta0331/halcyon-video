#!/usr/bin/env node
// Round 5B.3 HF3 IWER harness: per-hand Trigger source fidelity plus TAP/HOLD.
// This is not Quest visual proof.
...
const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), 'halcyon-jp4a-r5b3-hf3-'));
const port = Number(process.env.HALCYON_JP4A_PORT || 17436);
...
async function waitForServer() {
  ...
  throw new Error('JP4A HF3 harness server timeout');
}
...
try {
  ...
  await page.screenshot({ path: path.join(outDir, 'jp4a-round5b3-hf3-console.png') });
  await page.evaluate(() => {
  ...
  const firstRun = await page.evaluate(async () => {
    ...
    const seam = 'jp4a_stepHandedTrigger_runtime_source_routing';
    let t = 1;
  ...
  const evidence = {
    ...
    scope: 'PER_HAND_TRIGGER_SOURCE_AND_TAP_HOLD',
    ...
    iwerRayPickBoundary: 'Headless IWER controller pose is not a faithful Quest 12 m dual-controller geometry. Source fidelity uses the same chooseJp4aTriggerSource + stepJp4aHandedTrigger routing as XR runtime, with two distinct real shelf slots as left/right logical hits.',
    before,
    firstRun: {
  ...
  fs.writeFileSync(path.join(outDir, 'jp4a-round5b3-hf3-iwer.json'), JSON.stringify(evidence, null, 2));
  console.log(JSON.stringify({
    pass,
    evidence: 'docs/review/jp4a/jp4a-round5b3-hf3-iwer.json',
    liveShelfInvariant,
