#!/usr/bin/env node
// Round 5B.3 HF2 IWER harness: controller TAP/HOLD semantics plus same-page
// RESET/re-run. This is not Quest visual proof.
...
const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), 'halcyon-jp4a-r5b3-hf2-'));
const port = Number(process.env.HALCYON_JP4A_PORT || 17435);
...
async function waitForServer() {
  ...
  throw new Error('JP4A HF2 harness server timeout');
}
...
try {
  ...
  await page.screenshot({ path: path.join(outDir, 'jp4a-round5b3-hf2-console.png') });
  await page.evaluate(() => {
  ...
  const firstRun = await page.evaluate(async () => {
    ...
    const seam = 'jp4a_stepTrigger_runtime_state_machine';
    let t = 1;
    const tap = () => {
      const down = live.stepTrigger(true, t, true);
      t += 80;
      const up = live.stepTrigger(false, t, true);
      t += 80;
      return { down, up };
    };
    const hold = () => {
      const down = live.stepTrigger(true, t, true);
      t += HOLD;
      const crossed = live.stepTrigger(true, t, true);
      t += 40;
      const up = live.stepTrigger(false, t, true);
      t += 80;
    ...
    const beforeApproach = live.stepTrigger(true, t, true);
    t += HOLD - 1;
    const belowHold = live.stepTrigger(true, t, true);
    t += 1;
    const approachHold = live.stepTrigger(true, t, true);
    t += 40;
    const approachUp = live.stepTrigger(false, t, true);
    t += 80;
    const afterApproach = {
    ...
    return {
      entered, firstWorldRender: !!world, seam,
  ...
  const secondRun = await page.evaluate(async () => {
    ...
    const secondTap = live?.stepTrigger?.(true, 10, true);
    const secondUp = live?.stepTrigger?.(false, 90, true);
    await api?.exit?.();
    const completed = await waitUntil(() => window.__jp4aTestSnapshot?.()?.completedAt
      ? window.__jp4aTestSnapshot() : null, 5_000);
    return {
  ...
  const controllerOk = firstRun.afterLock?.locked === true
  ...
  const evidence = {
    ...
    iwerRayPickBoundary: 'Headless IWER controller pose is not a faithful Quest 12 m ray. Critical TAP/HOLD proof uses the same stepJp4aTrigger runtime state machine the XR poll path uses, with the first visible shelf slot as the captured hit.',
    before,
    firstRun: {
  ...
  fs.writeFileSync(path.join(outDir, 'jp4a-round5b3-hf2-iwer.json'), JSON.stringify(evidence, null, 2));
  console.log(JSON.stringify({
    pass,
    evidence: 'docs/review/jp4a/jp4a-round5b3-hf2-iwer.json',
    liveShelfInvariant,
